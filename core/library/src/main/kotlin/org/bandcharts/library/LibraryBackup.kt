...
data class LibraryBackupManifest(
    ...
    companion object {
         ...
         * and the difference is worth stating: a `.bcset` is handed between
...
object LibraryBackupCodec {
    ...
    fun fileName(deviceName: String?, now: Long): String {
        ...
        val label = deviceName
            ...
            ?.replace(Regex("[\\s_-]+"), "-")
            ?.ifEmpty { null }
        val base = listOfNotNull("BandCharts-backup", label, date.toString()).joinToString("-")
