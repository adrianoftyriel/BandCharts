...
class SetlistTest {
    ...
    fun `a bundle round trips through json`() {
        val bundle = SetlistCodec.bundle(set, exportedBy = "Jim", producer = "BandCharts 0.1.0", now = 1000L)
    ...
    fun `exported file names are safe`() {
        assertEquals(
            "Friday-at-the-Anchor.bcset",
            SetlistCodec.fileName(set),
        )
        assertEquals("setlist.bcset", SetlistCodec.fileName(set.copy(name = "///")))
        assertEquals("AC-DC-night.bcset", SetlistCodec.fileName(set.copy(name = "AC/DC night")))
    }
}
