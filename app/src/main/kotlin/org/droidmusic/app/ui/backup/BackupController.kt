...
import org.droidmusic.library.LibraryBackupCodec
...
class BackupController(
    ...
    fun backup() {
        scope.launch {
            ...
            val share = Intent(Intent.ACTION_SEND).apply {
                ...
                putExtra(Intent.EXTRA_SUBJECT, "DroidMusic backup")
                addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            }
            context.startActivity(
                Intent.createChooser(share, "Save DroidMusic backup")
    ...
    fun restore(uri: Uri) {
        scope.launch {
            ...
            if (manifest == null) {
                message = "That does not look like a DroidMusic backup."
                return@launch
            }
            if (!LibraryBackupCodec.canRead(manifest)) {
                message = "That backup was made by a newer version of DroidMusic. " +
