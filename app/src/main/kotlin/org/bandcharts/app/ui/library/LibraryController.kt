package org.bandcharts.app.ui.library
...
import org.bandcharts.app.data.WebChart
import org.bandcharts.library.FileKind
import org.bandcharts.library.LibraryIndex
import org.bandcharts.library.SongRef
import org.bandcharts.library.SourceKind
import org.bandcharts.library.SourceRef
import org.bandcharts.library.UltimateGuitar
import org.bandcharts.library.UltimateGuitarChart
import org.bandcharts.library.normaliseForMatching
import org.bandcharts.music.ChartAnalyzer
import org.bandcharts.music.Key
import org.bandcharts.music.SongParser
...
class LibraryController(
        ...
        scope.launch {
            ...
            if (skipped.isNotEmpty()) {
                lastError = buildString {
                    ...
                    append(". BandCharts opens PDFs, images, Word documents and any chart ")
    ...
    private suspend fun importChart(shared: String): String? {
        ...
        val url = UltimateGuitar.chartUrlIn(shared)
            ?: return "BandCharts imports chord charts from Ultimate Guitar links, " +
    ...
    private fun rewrite(song: SongRef, title: String, text: String): SongRef {
        val file = DocumentSources.managedFileOf(context, song)
            ?: throw IllegalStateException(
                "that chart lives in one of your own folders, and BandCharts only has " +
                    "read access to it",
            )
        file.writeText(text)
        return song.copy(
     ...
     *
     * Through [org.bandcharts.session.ChartShare.safeFileName] rather than a
    ...
    /**
     * Renames a chart, for BandCharts's purposes only. The file is not touched.
     *
        ...
        scope.launch {
            if (DocumentSources.deleteFile(context, song)) {
                ...
                lastError = "Could not delete ${song.bestTitle}. " +
                    "BandCharts has read-only access to that file, so deleting it has to be " +
    ...
    private suspend fun scan(source: SourceRef) {
        ...
        scanStatus = ""
        if (found.isEmpty()) {
            lastError = "Nothing readable in ${source.label}. BandCharts opens PDFs, images, " +
